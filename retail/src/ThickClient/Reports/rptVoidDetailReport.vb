...
Public Class rptVoidDetailReport
    ...
    Private Sub rptVoidReport_ReportStart(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.ReportStart
        ...
        Try
            ...
            txtDate.Text = dateFrom.ToString("dd-MM-yyyy") & " TO " & dateTo.ToString("dd-MM-yyyy")
            txtTime.Text = GetNow()
            txtUser.Text = IIf(Me.userId = 0, "ALL", objDA.GetUserName(Me.userId)) & " / " & IIf(Me.posId = 0, "ALL", posId)

            ' get transaction
            ds = Nothing
            ds = objDA.GetDetailedVoidReport(Me.userId, Me.dateFrom, Me.dateTo, Me.posId)
            Me.DataSource = ds.Tables(0)

        Catch ex As Exception
            Messenger.ShowError("Void Report Error!")
