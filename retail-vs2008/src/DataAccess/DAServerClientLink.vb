...
Public Class DAServerClientLink

    'Constants for Table field mappings
    Private Const CLASSNAME = "DAServerClientLink"
    ...
    Public Const TABLENAME = "app_server_client_link"
    Public Const NO_OF_COLUMNS As Integer = 5
    ...
    '* Create
    '**************************************************************************/
    Public Function Create(ByVal obj As DAServerClientLinkObject) As DAServerClientLinkObject
    ...
    '* Update
    '**************************************************************************/
    Public Sub Update(ByVal obj As DAServerClientLinkObject)
    ...
    '* GetObject
    '**************************************************************************/
    Public Function GetObject(ByVal id As Integer) As DAServerClientLinkObject
    ...
    '* GetObject
    '**************************************************************************/
    Public Function GetObject(ByVal row As DataRow) As DAServerClientLinkObject

        WriteToLogFile(CLASSNAME & " - In GetObject")

        Dim comm As New Commands
        Dim obj As DAServerClientLinkObject

        Try
            obj = New DAServerClientLinkObject
            obj.pkid = row(PKID)
            obj.branchId = row(BRANCH_ID)
            obj.tableName = row(TABLE_NAME)
            obj.serverKey = row(SERVER_KEY)
            obj.clientKey = row(CLIENT_KEY)
    ...
    '* ReplaceStatement
    '**************************************************************************/
    Public Function ReplaceStatement(ByVal strSQL As String, ByVal obj As DAServerClientLinkObject) As String

        strSQL = strSQL.Replace("{T}", TABLENAME)

        strSQL = strSQL.Replace("{1}", PKID)
        strSQL = strSQL.Replace("{2}", BRANCH_ID)
        strSQL = strSQL.Replace("{3}", TABLE_NAME)
        strSQL = strSQL.Replace("{4}", SERVER_KEY)
        strSQL = strSQL.Replace("{5}", CLIENT_KEY)

        strSQL = strSQL.Replace("{V1}", PreparedStatement(obj.pkid))
        strSQL = strSQL.Replace("{V2}", PreparedStatement(obj.branchId))
        strSQL = strSQL.Replace("{V3}", PreparedStatement(obj.tableName))
        strSQL = strSQL.Replace("{V4}", PreparedStatement(obj.serverKey))
        strSQL = strSQL.Replace("{V5}", PreparedStatement(obj.clientKey))
    ...
    '* GetObject
    '**************************************************************************/
    Public Function GetObject(ByVal serverKey As Long, ByVal strTableName As String) As DAServerClientLinkObject
        ...
        strSQL = strSQL.Replace("{T}", TABLENAME)
        strSQL = strSQL.Replace("{1}", SERVER_KEY)
        strSQL = strSQL.Replace("{V1}", serverKey)
        strSQL = strSQL.Replace("{2}", TABLE_NAME)
        strSQL = strSQL.Replace("{V2}", strTableName)
